...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 style="font-family: 'Playfair Display', serif; font-weight: 700; color: #1a1a1a; margin: 0;">Editar Empleado</h1>
            <p class="text-muted mb-0">Actualice los datos del empleado</p>
        </div>
        ...
        <div class="card border-0 shadow-sm" style="border-radius: 25px; overflow: hidden;">
            ...
            <div class="bg-dark text-white px-4 py-3" style="border-radius: 25px 25px 0 0;">
                <h5 class="mb-0 font-weight-bold">
                    <i class="fas fa-user-edit mr-2 text-warning"></i> Editar Empleado
                </h5>
                <small class="text-muted">Modifique los datos del empleado</small>
            </div>

            {{-- Cuerpo del formulario --}}
            <div class="card-body p-4">
                <form method="POST" action="{{ route('empleados.update', $empleado->IdEmpleado) }}" id="formEmpleadoEdit">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="puesto" class="form-control rounded-pill bg-light border-0 px-4" required>
                                ...
                                @foreach($puestos as $puesto)
                                    <option value="{{ $puesto }}" {{ old('puesto', $empleado->puesto) == $puesto ? 'selected' : '' }}>
                                        {{ $puesto }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" name="fecha_ingreso" class="form-control rounded-pill bg-light border-0 px-4" 
                                   value="{{ old('fecha_ingreso', $empleado->fecha_ingreso ? $empleado->fecha_ingreso->format('Y-m-d') : '') }}" required>
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="tipo_documento" class="form-control rounded-pill bg-light border-0 px-4">
                                ...
                                @foreach($tiposDocumento as $key => $tipo)
                                    <option value="{{ $key }}" {{ old('tipo_documento', $empleado->tipo_documento) == $key ? 'selected' : '' }}>
                                        {{ $tipo }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="numero_documento" class="form-control rounded-pill bg-light border-0 px-4" 
                                   value="{{ old('numero_documento', $empleado->numero_documento) }}" placeholder="Opcional">
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="telefono" class="form-control rounded-pill bg-light border-0 px-4" 
                                   value="{{ old('telefono', $empleado->telefono) }}" placeholder="Opcional">
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="email" name="email_personal" class="form-control rounded-pill bg-light border-0 px-4" 
                                   value="{{ old('email_personal', $empleado->email_personal) }}" placeholder="Opcional">
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="IdUsuario" class="form-control rounded-pill bg-light border-0 px-4">
                                ...
                                @foreach($usuariosDisponibles as $usuario)
                                    <option value="{{ $usuario->IdUsuario }}" {{ old('IdUsuario', $empleado->IdUsuario) == $usuario->IdUsuario ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="IdHotel" class="form-control rounded-pill bg-light border-0 px-4" required>
                                    ...
                                    @foreach($hoteles as $hotel)
                                        <option value="{{ $hotel->IdHotel }}" {{ old('IdHotel', $empleado->IdHotel) == $hotel->IdHotel ? 'selected' : '' }}>
                                            {{ $hotel->Nombre }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        @else
                            <input type="hidden" name="IdHotel" value="{{ $empleado->IdHotel }}">
                        @endif
                    </div>
...
<script>
    document.getElementById('formEmpleadoEdit').addEventListener('submit', function(e) {
