...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-5">
        <div>
            <h1 style="font-family: 'Playfair Display', serif; font-weight: 700; color: #1a1a1a; margin: 0;">Usuarios del Sistema</h1>
        ...
        @if(in_array(auth()->user()->IdRol, [1, 4]))
            <a href="{{ route('usuarios.create') }}" 
               ...
               style="font-size: 1rem;">
                <i class="fas fa-user-plus mr-2 text-warning"></i> Nuevo Usuario
            </a>
        @endif
    </div>
        ...
        <div class="card-body p-4">
            <form method="GET" id="formFiltros" class="row align-items-end">
                ...
                <div class="col-md-3 mb-3 mb-md-0">
                    ...
                    <select name="rol" class="form-control rounded-pill border-0 bg-light px-4">
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0 align-middle">
                ...
                <tbody class="bg-white">
                    ...
                    <tr class="{{ !$usuario->activo ? 'bg-light text-muted' : '' }}">
                        <td class="px-4 py-3">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <div class="font-weight-bold {{ $usuario->activo ? 'text-dark' : 'text-muted' }}">
                                        {{ $usuario->Username }}
                                    </div>
                                    @if(!$usuario->activo)
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="btn-group shadow-sm" style="border-radius: 10px; overflow: hidden;">
                                ...
                                @if(in_array(auth()->user()->IdRol, [1, 4]))
                                    ...
                                    @if($usuario->activo && auth()->user()->IdUsuario != $usuario->IdUsuario)
                                        <button type="button" 
                                                class="btn btn-light btn-sm px-3 btn-eliminar"
                                                data-url="{{ route('usuarios.destroy', $usuario->IdUsuario) }}"
                                    ...
                                    @elseif(!$usuario->activo)
                                        <button type="button" 
                                                ...
                                                data-url="{{ route('usuarios.reactivar', $usuario->IdUsuario) }}"
                                                data-nombre="{{ $usuario->Username }}"
                                                title="Reactivar"
                                                style="color: #28a745;">
    ...
    document.querySelectorAll('.btn-eliminar').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            SwalConfirmacion(
                ...
                'El usuario quedará como INACTIVO y no podrá iniciar sesión.',
                'warning',
                'Sí, desactivar'
            ).then((result) => {
                if (result.isConfirmed) {
                    ...
                    .catch(error => {
                        ...
                        SwalError('Error', 'Ocurrió un error al desactivar el usuario');
                    });
                }
            });
        });
    ...
    document.querySelectorAll('.btn-reactivar').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            SwalConfirmacionVerde(
                ...
                'El usuario volverá a estar disponible para iniciar sesión.',
                'Sí, reactivar'
            ).then((result) => {
                if (result.isConfirmed) {
                    fetch(url, {
                    ...
                    .catch(error => {
                        console.error('Error:', error);
                        SwalError('Error', 'Ocurrió un error al reactivar el usuario');
                    });
                }
            });
        });
