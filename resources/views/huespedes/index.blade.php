...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-5">
        <div>
            <h1 style="font-family: 'Playfair Display', serif; font-weight: 700; color: #1a1a1a; margin: 0;">Huéspedes</h1>
            <p class="text-muted mb-0">Gestión de clientes y huéspedes frecuentes</p>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0 align-middle">
                <thead class="bg-dark text-white">
                    <tr>
                        <th class="px-4 py-3 border-0">HUÉSPED</th>
                        <th class="py-3 border-0">DOCUMENTO</th>
                        <th class="py-3 border-0">CONTACTO</th>
                        <th class="py-3 border-0">NACIONALIDAD</th>
                ...
                <tbody class="bg-white">
                    ...
                    <tr class="{{ !$huesped->activo ? 'bg-light text-muted' : '' }}">
                        <td class="px-4 py-3">
                            <div class="d-flex align-items-center">
                                <div class="avatar-circle mr-3 bg-light text-dark font-weight-bold border-warning" style="border-width: 2px; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                                    {{ strtoupper(substr($huesped->Nombre, 0, 1)) }}{{ strtoupper(substr($huesped->Apellido, 0, 1)) }}
                                </div>
                                <div>
                                    <div class="font-weight-bold {{ $huesped->activo ? 'text-dark' : 'text-muted' }}">
                        ...
                        <td class="py-3">
                            <span class="badge badge-light px-3 py-2">
                                <i class="far fa-id-card mr-1 text-warning"></i>
                                {{ $huesped->TipoDocumento }}: {{ $huesped->NroDocumento }}
                            </span>
                        </td>
                        <td class="py-3">
                            @if($huesped->Telefono)
                                <div><i class="fas fa-phone text-muted mr-1 small"></i> {{ $huesped->Telefono }}</div>
                            @endif
                            @if($huesped->Email)
                                <div class="small text-muted"><i class="fas fa-envelope text-muted mr-1"></i> {{ $huesped->Email }}</div>
                            @endif
                        </td>
                        <td class="py-3">
                                ...
                                @if(in_array(auth()->user()->IdRol, [1, 4]))
                                    @if($huesped->activo)
                                        <button type="button" 
                                                ...
                                                data-url="{{ route('huespedes.destroy', $huesped->IdHuesped) }}"
                                                data-nombre="{{ $huesped->Nombre }} {{ $huesped->Apellido }}"
                                                title="Desactivar"
                                                style="color: #dc3545;">
                                    ...
                                    @else
                                        <button type="button" 
                                                ...
                                                data-url="{{ route('huespedes.reactivar', $huesped->IdHuesped) }}"
                                                data-nombre="{{ $huesped->Nombre }} {{ $huesped->Apellido }}"
                                                title="Reactivar"
                                                style="color: #28a745;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.btn-eliminar').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            SwalConfirmacion(
                '¿Desactivar huésped "' + nombre + '"?',
                'El huésped quedará como INACTIVO. No podrá hacer nuevas reservas pero se conservará su historial.',
                'warning',
                'Sí, desactivar'
            ).then((result) => {
                if (result.isConfirmed) {
                    ...
                    .catch(error => {
                        ...
                        SwalError('Error', 'Ocurrió un error al desactivar el huésped');
                    });
                }
            });
        });
    ...
    document.querySelectorAll('.btn-reactivar').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            SwalConfirmacionVerde(
                '¿Reactivar huésped "' + nombre + '"?',
                'El huésped volverá a estar disponible para hacer reservas.',
                'Sí, reactivar'
            ).then((result) => {
                if (result.isConfirmed) {
                    fetch(url, {
                    ...
                    .catch(error => {
                        console.error('Error:', error);
                        SwalError('Error', 'Ocurrió un error al reactivar el huésped');
                    });
                }
            });
        });
