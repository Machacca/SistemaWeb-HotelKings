...
@section('title', 'Canales de Reserva')
...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-5">
        <div>
            <h1 style="font-family: 'Playfair Display', serif; font-weight: 700; color: #1a1a1a; margin: 0;">Canales de Reserva</h1>
            <p class="text-muted mb-0">Gestión de orígenes de reservas</p>
        </div>
        
        @if(in_array(auth()->user()->IdRol, [1, 4]))
            <a href="{{ route('canales-reserva.create') }}" 
               class="btn btn-dark rounded-pill px-5 py-3 shadow-sm font-weight-bold" 
               style="font-size: 1rem;">
                <i class="fas fa-plus mr-2 text-warning"></i> Nuevo Canal
            </a>
        @endif
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0 align-middle">
                ...
                <tbody class="bg-white">
                    ...
                    <tr class="{{ !$canal->activo ? 'bg-light text-muted' : '' }}">
                        <td class="px-4 py-3">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <div class="font-weight-bold {{ $canal->activo ? 'text-dark' : 'text-muted' }}">
                                        {{ $canal->Nombre }}
                                    </div>
                                    @if(!$canal->activo)
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="btn-group shadow-sm" style="border-radius: 10px; overflow: hidden;">
                                <a href="{{ route('canales-reserva.show', $canal->IdCanal) }}" 
                                ...
                                @if(in_array(auth()->user()->IdRol, [1, 4]))
                                    <a href="{{ route('canales-reserva.edit', $canal->IdCanal) }}" 
                                       class="btn btn-light btn-sm px-3" 
                                       title="Editar"
                                       style="color: #26a53b;">
                                    ...
                                    @if($canal->activo)
                                        <button type="button" 
                                                ...
                                                data-url="{{ route('canales-reserva.destroy', $canal->IdCanal) }}"
                                                data-nombre="{{ $canal->Nombre }}"
                                                title="Desactivar"
                                                style="color: #dc3545;">
                                    ...
                                    @else
                                        <button type="button" 
                                                ...
                                                data-url="{{ route('canales-reserva.reactivar', $canal->IdCanal) }}"
                                                data-nombre="{{ $canal->Nombre }}"
                                                title="Reactivar"
                                                style="color: #28a745;">
                    ...
                    <tr>
                        <td colspan="4" class="text-center text-muted py-5">
                            <i class="fas fa-channels fa-3x mb-3 d-block"></i>
                            No se encontraron canales de reserva
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.btn-eliminar').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            SwalConfirmacion(
                '¿Desactivar canal "' + nombre + '"?',
                'Los canales desactivados no podrán seleccionarse en nuevas reservas.',
                'warning',
                'Sí, desactivar'
            ).then((result) => {
                if (result.isConfirmed) {
                    ...
                    .catch(error => {
                        ...
                        SwalError('Error', 'Ocurrió un error al desactivar el canal');
                    });
                }
            });
        });
    ...
    document.querySelectorAll('.btn-reactivar').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            SwalConfirmacionVerde(
                '¿Reactivar canal "' + nombre + '"?',
                'El canal estará nuevamente disponible para seleccionar en reservas.',
                'Sí, reactivar'
            ).then((result) => {
                if (result.isConfirmed) {
                    fetch(url, {
                    ...
                    .catch(error => {
                        console.error('Error:', error);
                        SwalError('Error', 'Ocurrió un error al reactivar el canal');
                    });
                }
            });
        });
